  ...
  class ImagePicker {
    ...
    render() {
      ...
      this.files.forEach((file, index) => {
        ...
        coverBtn.className = 'image-preview-cover-btn';
        coverBtn.textContent = 'Обложка';
        coverBtn.setAttribute('aria-label', `Сделать обложкой: ${file.name}`);
        coverBtn.addEventListener('click', (event) => {
          event.stopPropagation();
          this.setCoverIndex(index);
        });
        ...
        remove.addEventListener('click', (event) => {
          ...
          this.hideError();
          this.syncInput();
          const cover = this.readCoverIndex();
          if (cover >= this.files.length) {
            this.writeCoverIndex(0);
          } else if (cover > index) {
            this.writeCoverIndex(cover - 1);
          }
          this.render();
          if (window.refreshIcons) window.refreshIcons();
        });

        item.addEventListener('click', () => this.setCoverIndex(index));
        item.append(img, meta, coverBtn, remove);
    ...
    coverField() {
      return this.form?.querySelector('#cover-index-field') || document.getElementById('cover-index-field');
    }

    readCoverIndex() {
      const field = this.coverField();
      return field ? Math.max(0, parseInt(field.value || '0', 10) || 0) : 0;
    }

    writeCoverIndex(index) {
      const field = this.coverField();
      if (field) field.value = String(Math.max(0, index));
    }
    ...
    setCoverIndex(index) {
      ...
      const safe = Math.min(Math.max(index, 0), this.files.length - 1);
      this.writeCoverIndex(safe);
      this.applyCoverState();
    }

    applyCoverState() {
      if (!this.previewEl) return;
      const items = Array.from(this.previewEl.querySelectorAll('.image-preview-item'));
      if (!items.length) {
        this.writeCoverIndex(0);
        return;
      }
      let cover = this.readCoverIndex();
      ...
      items.forEach((item, i) => {
        ...
        const coverBtn = item.querySelector('.image-preview-cover-btn');
        if (isCover) {
          if (!badge) {
          ...
          badge?.remove();
          if (coverBtn) coverBtn.hidden = false;
        }
      });
    }
  }
